...
def criterion(inputs, target, num_classes, background_weight=1.0, focus_class=0):
    '''focus_class:int, choose one class to focus on training,
    set weight to the loss as 1 and others as 0 except background'''
    losses = {}
    if not focus_class:
        weight = torch.ones(num_classes)        
    else:
        weight = torch.zeros(num_classes)        
        weight[focus_class] = 1.0
